...
const messagesRoutes = function(db) {
  ...
  router.get("/", (req, res) => {
    ...
    let query = `
    SELECT messages.id AS message_id, messages.user_sender_id AS user_sender_id, messages.listing_id AS listing_id, messages.message AS message, messages.user_receiver_id AS user_receiver_id, users.name AS sender_name, users.user_pic AS user_pic
    FROM messages
